...
def main(args):
    ...
    #VIDEOSRC=''' videotestsrc is-live=true'''
    VIDEOSRC=''' v4l2src device=/dev/video0 '''

    RTPSINK = '''udpsink host=localhost port=5000 sync=false'''
    TEE_RTP = ''' tee name=t '''
    TEE_RAW = ''' tee name=r '''
    ...
    PIPELINE_DESC = VIDEOSRC + ''' ! ''' + VIDEO_RAW_CAPS + ''' ! videoconvert ! queue ! x264enc ! video/x-h264,profile=baseline ! h264parse ! rtph264pay pt=98 ! ''' + VIDEO_CAPS_RTP + ''' ! queue ! ''' + RTPSINK
    PIPELINE_DESC += ''' udpsrc port=5000 ! application/x-rtp,media=video,clock-rate=90000,payload=98 ! queue ! rtph264depay ! h264parse !  avdec_h264 ! queue !  videoconvert ! autovideosink'''
